...
func Nodes() *types.NetworkDeviceList {
	...
	// Location format: "City, AdminName, Country"
	devices = append(devices, createRouter("R1", "192.168.1.1", 1, "Tokyo, Tōkyō, Japan", 139.7495, 35.6870))
	devices = append(devices, createRouter("R2", "192.168.1.2", 2, "New York, New York, United States", -73.9249, 40.6943))
	devices = append(devices, createSwitch("SW1", "192.168.2.1", 3, "London, London, City of, United Kingdom", -0.1275, 51.5072))
	devices = append(devices, createSwitch("SW2", "192.168.2.2", 4, "São Paulo, São Paulo, Brazil", -46.6339, -23.5504))
	devices = append(devices, createFirewall("FW1", "192.168.3.1", 5, "Mumbai, Mahārāshtra, India", 72.8775, 19.0761))

	// Additional devices across the globe
	devices = append(devices, createRouter("R3", "192.168.1.3", 6, "Sydney, New South Wales, Australia", 151.2000, -33.8667))
	devices = append(devices, createRouter("R4", "192.168.1.4", 7, "Paris, Île-de-France, France", 2.3522, 48.8567))
	devices = append(devices, createSwitch("SW3", "192.168.2.3", 8, "Singapore, , Singapore", 103.8000, 1.3000))
	devices = append(devices, createSwitch("SW4", "192.168.2.4", 9, "Dubai, Dubayy, United Arab Emirates", 55.2972, 25.2631))
	devices = append(devices, createFirewall("FW2", "192.168.3.2", 10, "Toronto, Ontario, Canada", -79.3733, 43.7417))
	devices = append(devices, createRouter("R5", "192.168.1.5", 11, "Berlin, Berlin, Germany", 13.4050, 52.5200))
	devices = append(devices, createRouter("R6", "192.168.1.6", 12, "Moscow, Moskva, Russia", 37.6175, 55.7506))
	devices = append(devices, createSwitch("SW5", "192.168.2.5", 13, "Hong Kong, , Hong Kong", 114.2000, 22.3000))
	devices = append(devices, createSwitch("SW6", "192.168.2.6", 14, "Amsterdam, Noord-Holland, Netherlands", 4.8936, 52.3728))
	devices = append(devices, createFirewall("FW3", "192.168.3.3", 15, "Seoul, Seoul, Korea, South", 126.9833, 37.5667))
	devices = append(devices, createRouter("R7", "192.168.1.7", 16, "Los Angeles, California, United States", -118.4068, 34.1141))
	devices = append(devices, createRouter("R8", "192.168.1.8", 17, "Cape Town, Western Cape, South Africa", 18.4239, -33.9253))
	devices = append(devices, createSwitch("SW7", "192.168.2.7", 18, "Bangkok, Krung Thep Maha Nakhon, Thailand", 100.4942, 13.7525))
	devices = append(devices, createSwitch("SW8", "192.168.2.8", 19, "Mexico City, Ciudad de México, Mexico", -99.1333, 19.4333))
	devices = append(devices, createFirewall("FW4", "192.168.3.4", 20, "Cairo, Al Qāhirah, Egypt", 31.2358, 30.0444))
